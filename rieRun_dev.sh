...
docker run --rm -ti -v ~/.aws:/root/.aws -w /usr/src/task -v $(pwd):/usr/src/task/ -v /root/dev/JBMDLFesPOSystem/.aws-lambda-rie:/usr/src/rie --env-file ./.env -p 9080:8080  --entrypoint /usr/src/rie/aws-lambda-rie pna /usr/local/bin/pnpm aws-lambda-ric-esm index_dev.handler
